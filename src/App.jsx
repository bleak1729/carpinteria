...
// ─── SUPABASE CONFIG ───────────────────────────────────────────────────────────
const SB_URL = "https://xrgxeepcdbobtswndppi.supabase.co";
const SB_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhyZ3hlZXBjZGJvYnRzd25kcHBpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU0NjgyOTcsImV4cCI6MjA5MTA0NDI5N30.jrUAE1T-GpVDZ1aW3TasMXOWrPYeF4fdtyO-VIwDK7E";
...
// ─── PDF EXPORT ────────────────────────────────────────────────────────────────
async function exportPDF(budget, project, client) {
  ...
  fc("#b5600a");doc.rect(0,0,W,36,"F");fc("#ffffff");doc.rect(5,0,W-5,36,"F");fc("#b5600a");doc.rect(M,10,3,16,"F");
  doc.setFont("helvetica","bold");doc.setFontSize(20);tc("#2c2018");doc.text("CARPINTERIA",M+9,22);
  doc.setFont("helvetica","normal");doc.setFontSize(9);tc("#9c8e80");doc.text("PRESUPUESTO DE PROYECTO",M+9,30);
  doc.setFontSize(8);doc.text("Fecha: "+(budget.fecha||today()),W-M,18,{align:"right"});
  doc.text("N "+(budget.id||"").slice(0,8).toUpperCase(),W-M,26,{align:"right"});
  y=46;

  fc("#fdf8f2");doc.rect(M,y,CW,28,"F");sc("#e2d9cc");doc.setLineWidth(0.3);doc.rect(M,y,CW,28);
  doc.setFont("helvetica","bold");doc.setFontSize(8);tc("#9c8e80");doc.text("PROYECTO",M+5,y+8);
  doc.setFontSize(13);tc("#2c2018");doc.text(project?.nombre||"—",M+5,y+18);
  tc("#9c8e80");doc.text("CLIENTE",M+90,y+8);
  doc.setFont("helvetica","normal");doc.setFontSize(10);tc("#2c2018");doc.text(client?.nombre||"—",M+90,y+18);
  ...
  fc("#f5f0e8");doc.rect(W-M-30,y+7,28,12,"F");doc.setFont("helvetica","bold");doc.setFontSize(8);tc(bcfg.color);
  doc.text((bcfg.label||"").toUpperCase(),W-M-16,y+15,{align:"center"});
  y+=36;

  const tbl=(headers,rows,widths,ttl,ac)=>{
    if(!rows.length)return;
    doc.setFont("helvetica","bold");doc.setFontSize(9);const[r,g,b]=rgb(ac);doc.setTextColor(r,g,b);
    doc.text(ttl,M,y+4);y+=9;
    fc("#f5f0e8");doc.rect(M,y,CW,8,"F");sc("#e2d9cc");doc.setLineWidth(0.2);doc.rect(M,y,CW,8);
    ...
    y+=8;
    rows.forEach((row,ri)=>{
      const rh=8;if(y+rh>272){doc.addPage();y=18;}
      if(ri%2===0){fc("#fdf8f2");doc.rect(M,y,CW,rh,"F");}
      sc("#e2d9cc");doc.setLineWidth(0.15);doc.line(M,y+rh,M+CW,y+rh);
      let rx=M;
      row.forEach((cell,ci)=>{const last=ci===row.length-1;doc.setFont("helvetica",last?"bold":"normal");doc.setFontSize(8);tc(last?ac:"#2c2018");doc.text(String(cell),last?rx+widths[ci]-2:rx+3,y+5.5,last?{align:"right"}:{});rx+=widths[ci];});
      y+=rh;
    });y+=5;
  };

  const items=budget.items||[], mo=budget.mano_obra||[], col=budget.colaciones||[], ge=budget.gastos_extra||[];
  if(items.length) tbl(["DESCRIPCION","MATERIAL","MEDIDA","P.UNIT.","SUBTOTAL"],items.map(it=>[it.desc||"",it.matNombre||"",it.tipo==="m2"?`${Number(it.ancho).toFixed(2)}x${Number(it.largo).toFixed(2)}m`:it.tipo==="ml"?`${Number(it.cant).toFixed(2)}ml`:`${it.cant}u`,`${fmt(it.precio)}/${it.unidad}`,fmt(it.sub)]),[46,42,28,28,34],"MATERIALES","#b5600a");
  ...
  [["Materiales",fmt(sm),"#b5600a",sm===0],["Mano de obra",fmt(smo),"#1a5fa8",smo===0],["Colaciones",fmt(sc2),"#2d7a3a",sc2===0],["Gastos adicionales",fmt(sge),"#c05c00",sge===0],["Subtotal",fmt(stot),"#2c2018",false],[`Margen (${budget.margen||0}%)`,`+ ${fmt(mar)}`,"#b5600a",false]]
  ...
  fc("#fef3e2");doc.rect(tX,y,tW,16,"F");sc("#e8a045");doc.setLineWidth(0.5);doc.rect(tX,y,tW,16);fc("#b5600a");doc.rect(tX,y,4,16,"F");
  doc.setFont("helvetica","bold");doc.setFontSize(8);tc("#9c8e80");doc.text("TOTAL PROYECTO",tX+8,y+6);
  doc.setFontSize(15);tc("#b5600a");doc.text(fmt(tot),tX+tW-3,y+12,{align:"right"});y+=22;
  ...
  for(let i=1;i<=pg;i++){doc.setPage(i);sc("#e2d9cc");doc.setLineWidth(0.3);doc.line(M,286,W-M,286);doc.setFont("helvetica","normal");doc.setFontSize(7);tc("#9c8e80");doc.text(`Carpinteria - ${today()}`,M,291);doc.text(`Pag. ${i}/${pg}`,W-M,291,{align:"right"});}
...
function MatForm({ item, onSave, onClose }) {
  ...
  const save = async () => {
    if(!f.nombre||!f.precio) return;
    setSaving(true);
    try {
      const data = { nombre:f.nombre, tipo:f.tipo, precio:Number(f.precio), descripcion:f.desc, stock:Number(f.stock||0) };
      const result = ed ? await sb.update("materiales", item.id, data) : await sb.insert("materiales", data);
      onSave(result);
    } catch(e){ alert("Error: "+e.message); }
    setSaving(false);
  };
  return (<>
    <MHead title={ed?"Editar Material":"Nuevo Material"} onClose={onClose}/>
    <div style={{padding:20}}>
      ...
      <Field label="Stock disponible"><input style={SI} type="number" value={f.stock} onChange={e=>up("stock",e.target.value)} placeholder="0"/></Field>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:6}}>
        ...
        <button style={{...BP,opacity:saving?0.6:1}} onClick={save} disabled={saving}><Check size={14}/>{saving?"Guardando...":ed?"Guardar":"Agregar"}</button>
      </div>
    </div>
  </>);
}
...
function CliForm({ item, onSave, onClose }) {
  ...
  const save = async () => {
    if(!f.nombre) return;
    setSaving(true);
    try { const result = ed ? await sb.update("clientes", item.id, f) : await sb.insert("clientes", f); onSave(result); }
    catch(e){ alert("Error: "+e.message); }
    setSaving(false);
  };
  return (<>
    <MHead title={ed?"Editar Cliente":"Nuevo Cliente"} onClose={onClose}/>
    <div style={{padding:20}}>
      <Field label="Nombre completo"><input style={SI} value={f.nombre} onChange={e=>up("nombre",e.target.value)} placeholder="ej: Juan Pérez"/></Field>
      <Row cols={mobile?"1fr":"1fr 1fr"}>
...
function ProjForm({ item, clients, onSave, onClose }) {
  ...
  const up = (k,v) => setF(p=>({...p,[k]:v}));
  const save = async () => {
    if(!f.nombre||!f.cliente_id) return;
    setSaving(true);
    try {
      const data = { nombre:f.nombre, cliente_id:f.cliente_id, estado:f.estado, descripcion:f.desc, inicio:f.inicio||null, entrega:f.entrega||null, notas:f.notas };
      const result = ed ? await sb.update("proyectos", item.id, data) : await sb.insert("proyectos", data);
      onSave(result);
    } catch(e){ alert("Error: "+e.message); }
    setSaving(false);
  };
  return (<>
    <MHead title={ed?"Editar Proyecto":"Nuevo Proyecto"} onClose={onClose}/>
    <div style={{padding:20}}>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:6}}>
        <button style={BG} onClick={onClose}>Cancelar</button>
        <button style={{...BP,opacity:saving?0.6:1}} onClick={save} disabled={saving}><Check size={14}/>{saving?"Guardando...":ed?"Guardar":"Crear proyecto"}</button>
      </div>
    </div>
  </>);
}
...
function BudgetForm({ item, projects, materials, clients, onSave, onClose }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const data = { proyecto_id:f.proyecto_id, estado:f.estado, margen:Number(f.margen), subtotal, total, items:f.items, mano_obra:f.mano_obra, colaciones:f.colaciones, gastos_extra:f.gastos_extra, notas:f.notas, fecha:f.fecha };
      const result = ed ? await sb.update("presupuestos", item.id, data) : await sb.insert("presupuestos", data);
      onSave(result);
    } catch(e){ alert("Error: "+e.message); }
    setSaving(false);
  };
  ...
  const IRow=({label,det,val,onDel})=>(
    <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 11px",borderRadius:8,background:T.cardAlt,border:"1px solid "+T.border,marginBottom:5}}>
      <div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{label}</div><div style={{fontSize:11,color:T.muted}}>{det}</div></div>
      <div style={{fontWeight:700,color:T.text,fontSize:14,flexShrink:0}}>{val}</div>
      <button onClick={onDel} style={{background:"none",border:"1px solid "+T.border,borderRadius:5,cursor:"pointer",color:T.muted,padding:"3px 4px",display:"flex",flexShrink:0}}><Trash2 size={12}/></button>
    </div>
  );
  ...
  return (<>
    <MHead title={ed?"Editar Presupuesto":"Nuevo Presupuesto"} onClose={onClose}/>
    <div style={{padding:20,overflowY:"auto"}}>
      ...
      <Section icon={Package} color={T.amber} bg={T.amberBg} title="Materiales">
        <Field label="Seleccionar material"><select style={SI} value={cm.matId} onChange={e=>setCm(p=>({...p,matId:e.target.value}))}><option value="">— Seleccionar —</option>{materials.map(m=><option key={m.id} value={m.id}>{m.nombre} ({TIPOS[m.tipo]})</option>)}</select></Field>
        {mat&&<>
          <div style={{marginBottom:10,padding:"7px 11px",borderRadius:7,background:T.amberBg,border:"1px solid "+T.amberMid+"50",fontSize:13,color:T.sub}}>Precio: <strong style={{color:T.amber}}>{fmt(mat.precio)}</strong>/{mat.tipo==="ml"?"ml":mat.tipo==="m2"?"m2":"u"}</div>
...
// ─── VIEWS ─────────────────────────────────────────────────────────────────────
function Dashboard({ mats, clis, projs, buds, setView, setModal }) {
  const { mobile, tablet } = useMedia();
  const activos=projs.filter(p=>p.estado==="en_progreso").length, totalPres=buds.reduce((a,b)=>a+(b.total||0),0), aprobados=buds.filter(b=>b.estado==="aprobado").length, stockBajo=mats.filter(m=>m.stock<5).length;
  return (
    <div>
      <div style={{marginBottom:20}}>
        <h1 style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:mobile?26:32,color:T.text,lineHeight:1.1}}>Panel General</h1>
      ...
      <div style={{display:"grid",gridTemplateColumns:mobile?"1fr 1fr":tablet?"1fr 1fr":"repeat(4,1fr)",gap:12,marginBottom:18}}>
        {[{l:"Proyectos activos",v:activos,Icon:Folder,c:T.blue,bg:T.blueBg},{l:"Valor presupuestos",v:fmt(totalPres),Icon:FileText,c:T.amber,bg:T.amberBg},{l:"Clientes",v:clis.length,Icon:Users,c:T.green,bg:T.greenBg},{l:"Stock OK / Total",v:`${mats.length-stockBajo}/${mats.length}`,Icon:Package,c:T.orange,bg:T.orangeBg}]
          .map(({l,v,Icon,c,bg})=>(
            <Card key={l} style={{display:"flex",alignItems:"center",gap:12,padding:14}}>
              <div style={{width:42,height:42,borderRadius:10,background:bg,border:"1px solid "+c+"25",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon size={20} color={c}/></div>
              <div style={{minWidth:0}}><div style={{fontSize:mobile?19:23,fontWeight:700,color:T.text,lineHeight:1}}>{v}</div><div style={{fontSize:11,color:T.muted,marginTop:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{l}</div></div>
            </Card>
          ))}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:tablet?"1fr":"1.3fr 1fr",gap:16}}>
        <Card>
          ...
          :projs.slice(0,5).map(p=>{ const cli=clis.find(c=>c.id===p.cliente_id),est=ESTADOS[p.estado]; return (
            <div key={p.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:"1px solid "+T.border}}>
              <div style={{width:7,height:7,borderRadius:4,background:est.color,flexShrink:0}}/>
              <div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.nombre}</div><div style={{fontSize:11,color:T.muted}}>{cli?.nombre||"—"}</div></div>
        ...
        <Card>
          ...
          {[{l:"Nuevo proyecto",I:Folder,a:()=>setModal({t:"proj",d:null})},{l:"Nuevo presupuesto",I:FileText,a:()=>setModal({t:"bud",d:null})},{l:"Agregar material",I:Package,a:()=>setModal({t:"mat",d:null})},{l:"Nuevo cliente",I:Users,a:()=>setModal({t:"cli",d:null})}]
            .map(({l,I,a})=>(
              <button key={l} onClick={a} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:9,border:"1px solid "+T.border,background:T.bg,color:T.text,cursor:"pointer",fontSize:14,fontFamily:"inherit",width:"100%",marginBottom:7}}>
                <div style={{width:30,height:30,borderRadius:7,background:T.amberBg,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><I size={14} color={T.amber}/></div>{l}<ChevronRight size={13} style={{marginLeft:"auto",color:T.muted}}/>
              </button>
            ))}
        </Card>
      </div>
...
function Materials({ mats, reload, setModal }) {
  ...
  const del=async id=>{ if(!confirm("¿Eliminar este material?")) return; try{ await sb.delete("materiales",id); reload(); }catch(e){ alert("Error: "+e.message); } };
  return (
    <div>
      ...
      {mobile?(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {list.map(m=><Card key={m.id} style={{padding:14}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:10}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontWeight:700,color:T.text,fontSize:15,marginBottom:5}}>{m.nombre}</div>
                <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginBottom:4}}>
                  <span style={{padding:"2px 8px",borderRadius:20,fontSize:11,fontWeight:600,background:T.amberBg,color:T.amber,border:"1px solid "+T.amberMid+"40"}}>{TIPOS[m.tipo]}</span>
                  <span style={{fontSize:14,fontWeight:700,color:T.amber}}>{fmt(m.precio)}<span style={{color:T.muted,fontWeight:400,fontSize:11}}>/{m.tipo==="ml"?"ml":m.tipo==="m2"?"m2":"u"}</span></span>
                </div>
                <span style={{color:m.stock<5?T.red:m.stock<10?T.orange:T.green,fontWeight:600,fontSize:13}}>Stock: {m.stock}</span>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:5}}>
                <button onClick={()=>setModal({t:"mat",d:m})} style={BSM}><Edit2 size={12}/></button>
                <button onClick={()=>del(m.id)} style={{...BSM,color:T.red,border:"1px solid "+T.red+"30",background:T.redBg}}><Trash2 size={12}/></button>
              </div>
            </div>
          </Card>)}
        </div>
        ...
        <Card style={{padding:0,overflow:"hidden"}}>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",minWidth:480}}>
              <thead><tr style={{background:T.bg}}>{["Nombre","Tipo","Precio","Stock","Descripción",""].map(h=><th key={h} style={{padding:"10px 14px",textAlign:"left",fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:0.8,borderBottom:"1px solid "+T.border}}>{h}</th>)}</tr></thead>
              <tbody>{list.map((m,ri)=><tr key={m.id} style={{borderBottom:"1px solid "+T.border,background:ri%2===0?T.card:T.cardAlt}}>
                <td style={{padding:"10px 14px",fontWeight:600,color:T.text,fontSize:14}}>{m.nombre}</td>
                <td style={{padding:"10px 14px"}}><span style={{padding:"2px 9px",borderRadius:20,fontSize:11,fontWeight:600,background:T.amberBg,color:T.amber,border:"1px solid "+T.amberMid+"40"}}>{TIPOS[m.tipo]}</span></td>
                <td style={{padding:"10px 14px",fontWeight:700,color:T.amber,fontSize:14}}>{fmt(m.precio)}<span style={{color:T.muted,fontWeight:400,fontSize:11}}>/{m.tipo==="ml"?"ml":m.tipo==="m2"?"m2":"u"}</span></td>
                <td style={{padding:"10px 14px",fontSize:14,fontWeight:700,color:m.stock<5?T.red:m.stock<10?T.orange:T.green}}>{m.stock}</td>
                <td style={{padding:"10px 14px",color:T.muted,fontSize:13,maxWidth:180,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{m.descripcion}</td>
                <td style={{padding:"10px 14px"}}><div style={{display:"flex",gap:5}}><button onClick={()=>setModal({t:"mat",d:m})} style={BSM}><Edit2 size={12}/></button><button onClick={()=>del(m.id)} style={{...BSM,color:T.red,border:"1px solid "+T.red+"30",background:T.redBg}}><Trash2 size={12}/></button></div></td>
              </tr>)}</tbody>
            </table>
          </div>
          {list.length===0&&<p style={{padding:40,textAlign:"center",color:T.muted}}>No se encontraron materiales</p>}
        </Card>
      )}
    </div>
  );
...
function Clients({ clis, projs, reload, setModal }) {
  ...
  const del=async id=>{ if(!confirm("¿Eliminar este cliente?")) return; try{ await sb.delete("clientes",id); reload(); }catch(e){ alert("Error: "+e.message); } };
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:mobile?"1fr":"repeat(auto-fill,minmax(275px,1fr))",gap:14}}>
        {list.map((c,ci)=>{ const cp=projs.filter(p=>p.cliente_id===c.id),act=cp.filter(p=>p.estado==="en_progreso").length,ac=COLS[ci%COLS.length]; return (
          <Card key={c.id} style={{padding:14}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:12}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:42,height:42,borderRadius:21,background:ac+"18",border:"2px solid "+ac+"35",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,color:ac,fontSize:19,flexShrink:0}}>{c.nombre.charAt(0).toUpperCase()}</div>
                <div><div style={{fontWeight:700,color:T.text,fontSize:15}}>{c.nombre}</div><div style={{fontSize:12,color:T.muted}}>{cp.length} proyecto{cp.length!==1?"s":""}</div></div>
              </div>
              <div style={{display:"flex",gap:4}}><button onClick={()=>setModal({t:"cli",d:c})} style={BSM}><Edit2 size={12}/></button><button onClick={()=>del(c.id)} style={{...BSM,color:T.red,border:"1px solid "+T.red+"30",background:T.redBg}}><Trash2 size={12}/></button></div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {c.tel&&<div style={{fontSize:13,color:T.sub}}>📞 {c.tel}</div>}
              {c.email&&<div style={{fontSize:13,color:T.sub}}>✉ {c.email}</div>}
              {c.dir&&<div style={{fontSize:13,color:T.sub}}>📍 {c.dir}</div>}
            </div>
            {act>0&&<div style={{marginTop:10,padding:"5px 10px",borderRadius:6,background:T.blueBg,border:"1px solid "+T.blue+"30",fontSize:12,color:T.blue}}>{act} proyecto{act!==1?"s":""} en progreso</div>}
          </Card>
        );})}
      </div>
...
function Projects({ projs, clis, buds, reload, setModal }) {
  ...
  const del=async id=>{ if(!confirm("¿Eliminar este proyecto?")) return; try{ await sb.delete("proyectos",id); reload(); }catch(e){ alert("Error: "+e.message); } };
  return (
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
        {[["all","Todos",null],...Object.entries(ESTADOS).map(([k,v])=>[k,v.label,v.color])].map(([v,l,dot])=>(
          <button key={v} onClick={()=>setFt(v)} style={{...BSM,background:ft===v?T.amberBg:T.white,color:ft===v?T.amber:T.muted,border:"1.5px solid "+(ft===v?T.amberMid+"60":T.border)}}>
            {dot&&<span style={{width:7,height:7,borderRadius:4,background:dot,display:"inline-block"}}/>}{l}
          </button>
        ))}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {list.map(p=>{ const cli=clis.find(c=>c.id===p.cliente_id),bud=buds.find(b=>b.proyecto_id===p.id),est=ESTADOS[p.estado]; return (
          <Card key={p.id} style={{padding:14}}>
            <div style={{display:"flex",gap:12}}>
              <div style={{width:4,alignSelf:"stretch",borderRadius:2,background:est.color,flexShrink:0}}/>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",alignItems:mobile?"flex-start":"center",gap:8,marginBottom:5,flexDirection:mobile?"column":"row"}}>
                  <span style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:18,color:T.text}}>{p.nombre}</span>
                  <Badge label={est.label} color={est.color} bg={est.bg}/>
                </div>
                <div style={{fontSize:13,color:T.muted,marginBottom:3}}><strong style={{color:T.sub}}>{cli?.nombre||"—"}</strong>{p.entrega&&` · Entrega: ${p.entrega}`}</div>
                {p.descripcion&&<div style={{fontSize:13,color:T.muted,marginBottom:3}}>{p.descripcion}</div>}
                {bud&&<div style={{marginTop:8,display:"inline-flex",gap:10,fontSize:12,padding:"5px 10px",background:T.amberBg,borderRadius:7,border:"1px solid "+T.amberMid+"40",flexWrap:"wrap"}}>
                  <span style={{color:T.sub}}>Presupuesto: <strong style={{color:T.amber}}>{fmt(bud.total)}</strong></span>
                  <Badge label={BESTADOS[bud.estado]?.label||bud.estado} color={BESTADOS[bud.estado]?.color} bg={BESTADOS[bud.estado]?.bg}/>
                </div>}
              </div>
              <div style={{display:"flex",flexDirection:mobile?"column":"row",gap:5,flexShrink:0}}>
                <button onClick={()=>setModal({t:"proj",d:p})} style={BSM}><Edit2 size={12}/></button>
                <button onClick={()=>del(p.id)} style={{...BSM,color:T.red,border:"1px solid "+T.red+"30",background:T.redBg}}><Trash2 size={12}/></button>
              </div>
            </div>
          </Card>
        );})}
...
function Budgets({ buds, projs, clis, mats, reload, setModal }) {
  const { mobile } = useMedia();
  const [exp,setExp]=useState(null);
  const del=async id=>{ if(!confirm("¿Eliminar?")) return; try{ await sb.delete("presupuestos",id); reload(); }catch(e){ alert("Error: "+e.message); } };
  const doExport=async b=>{ setExp(b.id); try{ const pj=projs.find(p=>p.id===b.proyecto_id),cl=pj?clis.find(c=>c.id===pj.cliente_id):null; await exportPDF(b,pj,cl); }catch(e){ alert("Error PDF: "+e.message); } setExp(null); };
  if(buds.length===0) return (
    <div>
      ...
      <Card style={{textAlign:"center",padding:50}}>
        <div style={{width:60,height:60,borderRadius:30,background:T.amberBg,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px"}}><FileText size={26} color={T.amber}/></div>
        <p style={{color:T.muted,fontSize:15,marginBottom:18}}>Aún no hay presupuestos</p>
        <button style={{...BP,margin:"0 auto"}} onClick={()=>setModal({t:"bud",d:null})}><Plus size={14}/>Crear presupuesto</button>
      </Card>
    </div>
  );
  return (
    <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {buds.map(b=>{ const pj=projs.find(p=>p.id===b.proyecto_id),cl=pj?clis.find(c=>c.id===pj.cliente_id):null,cfg=BESTADOS[b.estado]||BESTADOS.borrador;
          const sm=(b.items||[]).reduce((a,i)=>a+i.sub,0),smo=(b.mano_obra||[]).reduce((a,i)=>a+i.sub,0),sc=(b.colaciones||[]).reduce((a,i)=>a+i.sub,0),sg=(b.gastos_extra||[]).reduce((a,g)=>a+g.monto,0);
          return (
            <Card key={b.id} style={{padding:14}}>
              <div style={{display:"flex",flexDirection:mobile?"column":"row",gap:12}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5,flexWrap:"wrap"}}>
                    <span style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:18,color:T.text}}>{pj?.nombre||"Proyecto eliminado"}</span>
                    <Badge label={cfg.label} color={cfg.color} bg={cfg.bg}/>
                  </div>
                  <div style={{fontSize:13,color:T.muted,marginBottom:8}}>{cl?.nombre||"—"} · {b.fecha}</div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    {[[sm,"Materiales",T.amber,T.amberBg],[smo,"M.O.",T.blue,T.blueBg],[sc,"Colac.",T.green,T.greenBg],[sg,"Gastos",T.orange,T.orangeBg]].filter(([v])=>v>0).map(([v,l,c,bg])=>(
                      <span key={l} style={{fontSize:12,padding:"3px 9px",borderRadius:20,background:bg,color:c,fontWeight:600,border:"1px solid "+c+"25"}}>{l}: {fmt(v)}</span>
                    ))}
                  </div>
                </div>
                <div style={{display:"flex",flexDirection:mobile?"row":"column",alignItems:mobile?"center":"flex-end",justifyContent:"space-between",gap:8,flexShrink:0}}>
                  <div style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:mobile?22:26,color:T.amber}}>{fmt(b.total||0)}</div>
                  <div style={{display:"flex",gap:6}}>
                    <button onClick={()=>doExport(b)} disabled={!!exp} style={{...BP,padding:"7px 12px",fontSize:12,opacity:exp===b.id?0.6:1,background:T.green}}>{exp===b.id?"...":(<><Download size={12}/>{!mobile&&" PDF"}</>)}</button>
                    <button onClick={()=>setModal({t:"bud",d:b})} style={BSM}><Edit2 size={12}/></button>
                    <button onClick={()=>del(b.id)} style={{...BSM,color:T.red,border:"1px solid "+T.red+"30",background:T.redBg}}><Trash2 size={12}/></button>
                  </div>
                </div>
              </div>
            </Card>
...
export default function App() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setMats(m); setClis(c); setProjs(p); setBuds(b);
    } catch(e) {
      setError("No se pudo conectar con Supabase. Verifica que las tablas estén creadas.");
      console.error(e);
    }
    setLoading(false);
  };

  useEffect(() => { loadAll(); loadJsPDF(); }, []);
  ...
  // Modal save handlers
  const handleSaveMat = async (result) => { await loadAll(); setModal(null); showToast(result ? "Material guardado ✓" : "Guardado"); };
  const handleSaveCli = async (result) => { await loadAll(); setModal(null); showToast("Cliente guardado ✓"); };
  const handleSaveProj= async (result) => { await loadAll(); setModal(null); showToast("Proyecto guardado ✓"); };
  const handleSaveBud = async (result) => { await loadAll(); setModal(null); showToast("Presupuesto guardado ✓"); };
  ...
  if (loading) return (
    <div style={{ background:T.bg, height:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans',sans-serif", gap:12 }}>
      ...
      <p style={{ color:T.muted, fontSize:14 }}>Conectando con Supabase...</p>
  ...
  if (error) return (
    <div style={{ background:T.bg, height:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans',sans-serif", padding:24, gap:16 }}>
      <div style={{ width:60,height:60,borderRadius:30,background:T.redBg,display:"flex",alignItems:"center",justifyContent:"center" }}><AlertTriangle size={28} color={T.red}/></div>
      ...
      <p style={{ color:T.muted, fontSize:13, textAlign:"center", maxWidth:400 }}>Asegúrate de haber ejecutado el SQL de creación de tablas en tu proyecto Supabase.</p>
      <button style={BP} onClick={loadAll}><RefreshCw size={14}/>Reintentar</button>
    </div>
  );

  const ctx = { mats, clis, projs, buds, reload:loadAll, setModal, setView };
  ...
  return (
    <>
      ...
      <div style={{ display:"flex", height:"100vh", background:T.bg, fontFamily:"'DM Sans',sans-serif", color:T.text, overflow:"hidden" }}>
        {!mobile && (
          <aside style={{ width:sidebar?220:60, background:T.white, borderRight:"1px solid "+T.border, display:"flex", flexDirection:"column", transition:"width 0.2s", overflow:"hidden", flexShrink:0, boxShadow:"1px 0 4px rgba(44,32,24,0.05)" }}>
            <div style={{ padding:"15px 13px", borderBottom:"1px solid "+T.border, display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ width:34,height:34,background:T.amber,borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
                <span style={{ fontFamily:"'DM Serif Display',Georgia,serif", fontSize:18, color:"#fff" }}>C</span>
              </div>
              {sidebar&&<div><div style={{ fontFamily:"'DM Serif Display',Georgia,serif", fontSize:15, color:T.text, lineHeight:1 }}>Carpintería</div><div style={{ fontSize:9,color:T.muted,letterSpacing:0.5,marginTop:2 }}>SUPABASE · EN VIVO</div></div>}
            </div>
            <nav style={{ flex:1, padding:"10px 7px" }}>
              {NAV.map(({ id,Icon,label }) => { const on=view===id; return (
                <button key={id} onClick={()=>setView(id)} style={{ width:"100%",display:"flex",alignItems:"center",gap:10,padding:"10px 10px",borderRadius:9,border:"none",cursor:"pointer",background:on?T.amberBg:"transparent",color:on?T.amber:T.sub,marginBottom:3,textAlign:"left",fontFamily:"inherit",fontSize:14,fontWeight:on?600:400,borderLeft:on?"3px solid "+T.amber:"3px solid transparent" }}>
                  <Icon size={17} style={{ flexShrink:0 }}/>{sidebar&&<span>{label}</span>}
                </button>
              );})}
            </nav>
            <button onClick={()=>setSidebar(s=>!s)} style={{ margin:"10px 8px",padding:8,borderRadius:8,border:"1px solid "+T.border,cursor:"pointer",background:T.bg,color:T.muted,display:"flex",alignItems:"center",justifyContent:"center" }}>
              <Menu size={15}/>
            </button>
          </aside>
        )}
        ...
        <main style={{ flex:1, overflow:"auto", padding:mobile?"14px 14px 80px":"28px 36px", background:T.bg }}>
          {mobile&&<div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16 }}>
            <div style={{ display:"flex",alignItems:"center",gap:8 }}>
              <div style={{ width:30,height:30,background:T.amber,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center" }}><span style={{ fontFamily:"'DM Serif Display',Georgia,serif",fontSize:16,color:"#fff" }}>C</span></div>
              <span style={{ fontFamily:"'DM Serif Display',Georgia,serif",fontSize:15,color:T.text }}>Carpintería</span>
            </div>
            <button onClick={loadAll} style={{ ...BSM, color:T.muted }}><RefreshCw size={13}/></button>
          </div>}
          ...
          {view==="budgets"   && <Budgets   buds={buds} projs={projs} clis={clis} mats={mats} reload={loadAll} setModal={setModal}/>}
        </main>
      </div>
      ...
      {modal && (
        <Modal onClose={()=>setModal(null)} wide={modal.t==="bud"}>
          {modal.t==="mat"  && <MatForm    item={modal.d} onSave={handleSaveMat}  onClose={()=>setModal(null)}/>}
          {modal.t==="cli"  && <CliForm    item={modal.d} onSave={handleSaveCli}  onClose={()=>setModal(null)}/>}
          {modal.t==="proj" && <ProjForm   item={modal.d} clients={clis} onSave={handleSaveProj} onClose={()=>setModal(null)}/>}
          {modal.t==="bud"  && <BudgetForm item={modal.d} projects={projs} materials={mats} clients={clis} onSave={handleSaveBud} onClose={()=>setModal(null)}/>}
        </Modal>
      )}
    </>
  );
